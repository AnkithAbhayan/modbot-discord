...
class self_assign_roles(commands.Cog):
    ...
    async def get_role(self,ctx,*args):
        ...
        if developer_role not in ctx.author.roles:
            get_dev_role_embed = discord.Embed(title="No {developer_role.mention} role.",description="Hi! If you require specific language roles, you have to get the @developer role first. This role gives you access to channels related to programming.",color=data["error_colour"])
        ...
        if array[1] in data["user_roles"]:
            ...
            if role in ctx.author.roles:
                already_have_the_role = discord.Embed(title="You Already have the role.",description=f"It looks like you already have the {role.mention} role.",color=data["error_colour"])
                ctx.channel.send(embed=already_have_the_role)
                return
            await ctx.author.add_roles(role)
            got_the_role_embed = discord.Embed(title=f"{role.mention} role added!",description=f"You have now got the {role.mention} role! :tada:",color=data["standard_colour"])
            await ctx.channel.send(embed=got_the_role_embed)
        else:
            error_in_role_name = discord.Embed(title="Invalid role.",description=f"The role name you entered is incorrect!\n Correct usage:\n ```\n$get_role <role_name>\n```\nHere are all the valid role names: `{','.join(data['user_roles'])}`",color=data["error_colour"])
