    ...
    async def get_role(self, ctx, *args):
        ...
        if len(array) == 1:
            help_get_role = discord.Embed(
                ...
                description=f"This command lets you self assign roles to your user so that you can access the full benefits!\n**Usage**:\n```\n$get_role <role_name>```\n**Example**: `$get_role developer` gives you access to all programming related channels!\n **All self-assignable roles**: `{', '.join(data['user_roles'])}`",
                color=my_bot.standard_colour,
            )
        ...
        if developer_role not in ctx.author.roles:
            if array[1] == "developer":
                ...
                got_the_role_embed = discord.Embed(
                    title=f":white_check_mark: role added!",
